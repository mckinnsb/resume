...
import type { DisplayState } from "./reducer";
...
describe("reducer", () => {
  ...
  test("adds characters to non-visible places on REFRESH_DISPLAY", () => {
    let state: DisplayState = {
      main: "dragons are mean",
      left: "score",
      right: "oh god",
      inputting: true,
      inputting_at: 3
    ...
    let newState = displayReducer(state, refreshDisplay());

    let expected: DisplayState = {
      main: " dragons are mean",
      left: "score ",
      right: " oh god",
      inputting: true,
